...
class RelayActionTest extends TestCase
{
    ...
    /** @test */
    public function subscriber_with_hashtags_relay_action_dispatches_relayed_notifications()
    {
        /*** arrange ***/
        Notification::fake();
    ...
    /** @test */
    public function subscriber_without_hashtags_relay_action_dispatches_relayed_notifications()
    {
        /*** arrange ***/
        Notification::fake();
        $spokesman = Contact::create(['mobile' => '09654444444'])
        ...
        /*** assert ***/
        Notification::assertSentTo(Contact::bearing($sms->origin->mobile), Acknowledged::class);
        Notification::assertNotSentTo($spokesman, Hashtags::class);
        Notification::assertNotSentTo($listener1, Hashtags::class);
        Notification::assertNotSentTo($listener2, Hashtags::class);
        Notification::assertNotSentTo($listener3, Hashtags::class);
        Notification::assertNotSentTo($listener4, Hashtags::class);
        Notification::assertNotSentTo($listener5, Hashtags::class);
        $mobiles = Setting::get('forwarding.mobiles');
        foreach ($mobiles as $mobile) {
            $contact = Contact::bearing($mobile);
